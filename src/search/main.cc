...
extern "C" double doAction(double s[], int n, int* action);
extern "C" void connectToServer(char*, char*, int, struct RDDLInstance*, void(*fp1)(), void(*fp2)(), int(*fp3)(double* s));

SearchEngine* __lib;

double doAction(double s[], int n, int* action) {
    vector<double> v(s, s+n);
    PDState curr = State(v, SearchEngine::horizon);
    const std::vector<int>& validActions = __lib->getApplicableActions(curr);
    int a = (*action);
    ...
    double rwd = 0;
    __lib->calcReward(curr, aindex, rwd);
    PDState nxt;
    __lib->calcSuccessorState(curr, aindex, nxt);
    for (unsigned int i = 0; i < State::numberOfProbabilisticStateFluents; ++i) {
    	nxt.sample(i);
    }
    for (int i=0; i<State::numberOfDeterministicStateFluents; i++) {
    	s[i] = nxt.deterministicStateFluent(i);
    }
    for (int i=0; i<State::numberOfProbabilisticStateFluents; i++) {
    	s[i+State::numberOfDeterministicStateFluents] = nxt.probabilisticStateFluent(i);
    }
    return rwd;
}


void connectToServer(char* problem, char* host, int port, struct RDDLInstance* r, void(*fp1)(), void(*fp2)(), int(*fp3)(double*)) {
    Stopwatch totalTime;
    string plannerDesc = "[PROST -s 1 -se [IPPC2014]]";
    IPPCClient* client = new IPPCClient(host, port);
    client->run(problem, plannerDesc, r, fp1, fp2, fp3);
    cout << "PROST complete running time: " << totalTime << endl;
    delete r->initial_state;
    delete client;
    delete __lib;
}
