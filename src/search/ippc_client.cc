...
void IPPCClient::run(string const& input, string& plannerDesc, struct RDDLInstance* r, void(*fp1)(), void(*fp2)(), int(*fp3)(double*)) {
    ...
    //Murugeswari
    State j = State(SearchEngine::initialState);
    r->num_state_vars = State::numberOfDeterministicStateFluents + State::numberOfProbabilisticStateFluents;
    r->num_action_vars = SearchEngine::numActionFluents;
    r->num_enum_actions = SearchEngine::numberOfActions;
    r->horizon = SearchEngine::horizon;
    r->num_rounds = numberOfRounds;
    r->remaining_time = remainingTime;
    r->initial_state = new int[r->num_state_vars];
    for (int i=0; i<State::numberOfDeterministicStateFluents; i++) {
        (r->initial_state)[i] = j.deterministicStateFluent(i);
    }
    for (int i=0; i<State::numberOfProbabilisticStateFluents; i++) {
        (r->initial_state)[i+State::numberOfDeterministicStateFluents] = j.probabilisticStateFluent(i);
    }
    __lib = planner->searchEngine;
    r->min_reward = __lib->rewardCPF->getMinVal();
    r->max_reward = __lib->rewardCPF->getMaxVal();
    fp1();
    fp2();
    //:
    ...
    //Murugeswari
    double* nextStateArray = new double[stateVariableIndices.size()];
    ...
    for (unsigned int currentRound = 0; currentRound < numberOfRounds;
        ...
        while (true) {
            ...
            copy(nextState.begin(), nextState.end(), nextStateArray);
            int actionIndex = fp3(nextStateArray);
            vector<string> nextActions = planner->plan(actionIndex);
            //:
...
void IPPCClient::executeParser(string const& problemName,
    ...
    int result =
        std::system(std::string("./rddl-parser " + homedir + "/parser_in.rddl " + homedir + "/ -ipc2018 1").c_str());
    if (result != 0) {
