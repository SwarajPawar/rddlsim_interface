...
//Murugeswari
vector<string> ProstPlanner::plan(int a) {
    const std::vector<int>& validActions = searchEngine->getApplicableActions(currentState);
    int actionIndex = 0;
    if (validActions[a] == a) {
        actionIndex = a;
    }
    chosenActionIndices[currentRound][currentStep] = actionIndex;
    int& chosenActionIndex = chosenActionIndices[currentRound][currentStep];
    vector<string> result;
    for (size_t i = 0; i < SearchEngine::actionStates[chosenActionIndex]
                               .scheduledActionFluents.size();
